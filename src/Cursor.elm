...
toList a = 
    case a of 
        Cursor left middle right -> List.reverse left ++ (middle::right)
...
back cursor =
    case cursor of
        Cursor (last::left) middle right -> Just (withSelectedElement left last (middle::right))
        Cursor [] middle right -> Nothing
...
length cursor =
    case cursor of
        Cursor left _ right -> 1 + List.length left + List.length right